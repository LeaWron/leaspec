...
当 leaspec 流程需要用户选择、确认或多选输入时，必须先确认当前 Agent 运行环境是否提供可调用的结构化交互工具（例如 Claude Code 的 `AskUserQuestion` 或等价能力）。

- 如果当前工具列表中没有可用的结构化交互工具，且用户没有提前提供完整决策或明确授权使用默认值，不得把流程退化为 shell TTY prompt，也不得用默认值继续执行会产生文件写入、脚本运行或 git 提交的步骤
- 一旦检测到无法交互，立即停止在第一个副作用之前，向用户说明当前模式无法完成该 leaspec 交互步骤，并提示切换到支持交互的模式或重新发起请求时一次性提供所需参数
